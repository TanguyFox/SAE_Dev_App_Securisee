...
class CreateProfilAction extends Action
{
    ...
    public function execute(): string
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === "GET") {
            $html = <<<END
                <form method='post' action='?action=create-profil' enctype="multipart/form-data">
                <label>Nom du profil : <input type="text" name="name_profil"></label>
                <label>Vous pouvez insérez une image de profil !(format accepté : jpeg, png) <input type="file" name="inputfile"></label>
            ...
            if (($_FILES['inputfile']['error'] === UPLOAD_ERR_OK) &&
                ...
                if (move_uploaded_file($tmp, $dest)) {
                    if(isset($_SESSION['user'])){
                        $name = filter_var($_POST['name_profil'],FILTER_SANITIZE_SPECIAL_CHARS);
                        $user_account = new Account($dest,$name);
                        $user_account->insertAccount();
                        $_SESSION['account']=$user_account;
                        $html  .= "Création du compte réussi ! <a href='?action=accueil-catalogue'>Découvrez-le !</a>";
                    }
                } else {
